...
void DrawSmoke3DGPU(smoke3ddata *smoke3di){
  ...
  case -1:
    ...
    slice_beg = is1;
    slice_end = is2;
    for(iii = slice_beg; iii<slice_end; iii += smoke3d_skip_frontback){
      i = iii;
      ...
      for(k = ks1; k<ks2; k+=smoke3d_skip_vert){
        ...
        znode[2] = z3;
        znode[3] = z3;
        for(j = js1; j<js2; j+=smoke3d_skip_horiz){
          int j2;
  ...
  case -2:
    ...
    for(jjj = slice_beg; jjj<slice_end; jjj += smoke3d_skip_frontback){
      ...
      for(k = ks1; k<ks2; k+=smoke3d_skip_vert){
        ...
        znode[2] = z3;
        znode[3] = z3;
        for(i = is1; i<is2; i+=smoke3d_skip_horiz){
          int i2;
...
void SkipSmokeFrames(FILE *SMOKE3DFILE, smoke3ddata *smoke3di, int nsteps){
  int i;
  int skip_local = 0;
...
FILE *FopenSmokeSizefile(char *smokefile, int fortran_skip){
  FILE *SMOKE_SIZE_FILE;
  ...
  SMOKE_SIZE_FILE = FOPEN_2DIR(smoke_sizefilename, "r");
  ...
  if(SMOKE_SIZE_FILE != NULL)return SMOKE_SIZE_FILE;
...
int GetSmoke3DSizes(smoke3ddata *smoke3di, int fortran_skip, char *smokefile, 
  unsigned char **times_map_ptr, float **times_ptr, int **use_smokeframe,
  ...
  rewind(SMOKE_SIZE_FILE);
  if(nframes_found <= 0){
    *ntimes_found = 0;
    *ntimes_full = 0;
    fclose(SMOKE_SIZE_FILE);
    printf("***warning: %i smoke frames found <= 0\n",nframes_found);
    return 1;
  }
...
int GetSmoke3DVersion2(smoke3ddata *smoke3di){
  ...
  SKIP_SMOKE(SMOKE3DFILE);fread(nxyz, 4, 8, SMOKE3DFILE); SKIP_SMOKE(SMOKE3DFILE);
  if(smoke3di->version == 2){
  ...
  }
  {
    ...
    SKIP_SMOKE(SMOKE3DFILE);fread(&time_local, 4, 1, SMOKE3DFILE);SKIP_SMOKE(SMOKE3DFILE);
    if(feof(SMOKE3DFILE)==0){
      SKIP_SMOKE(SMOKE3DFILE);fread(nchars, 4, 2, SMOKE3DFILE);SKIP_SMOKE(SMOKE3DFILE);
    }
  }
...
int SetupSmoke3D(smoke3ddata *smoke3di, int load_flag, int iframe_arg, int *errorcode_arg){
  ...
  if(GetSmoke3DSizes(smoke3di, fortran_skip, smoke3di->file, &smoke3di->times_map, &smoke3di->times, &smoke3di->use_smokeframe,
...
FILE *OpenSoot3DFile(char *s3dfile, char *s3ddfile){
...
FILE_SIZE ReadSmoke3D(int time_frame,int ifile_arg,int load_flag, int first_time, int *errorcode_arg){
  ...
  if(smoke3di->type == SOOT_index){
    SMOKE3DFILE = OpenSoot3DFile(smoke3di->file, smoke3di->smoke_density_file);
  }
  else{
  ...
  SKIP_SMOKE(SMOKE3DFILE);fread(nxyz_local,4,8,SMOKE3DFILE);SKIP_SMOKE(SMOKE3DFILE);
  if(smoke3di->version == 2){
    float bounds_local[2];
  ...
  for(i=frame_start_local; i<frame_end_local; i++){
    ...
    if(feof(SMOKE3DFILE)!=0||(use_tload_end==1&&time_local>global_scase.tload_end)){
    ...
    if(smoke3di->version == 2){
      fread(nchars_local, 4, 3, SMOKE3DFILE);
    }
    else{
      fread(nchars_local, 4, 2, SMOKE3DFILE);
    }
    SKIP_SMOKE(SMOKE3DFILE);
    ...
    if(smoke3di->use_smokeframe[i]==1){
      ...
      nframes_found_local++;
      SKIP_SMOKE(SMOKE3DFILE);fread(smoke3di->smokeframe_comp_list[iii],1,smoke3di->nchars_compressed_smoke[iii],SMOKE3DFILE); SKIP_SMOKE(SMOKE3DFILE);
      ...
      SKIP_SMOKE(SMOKE3DFILE);fseek(SMOKE3DFILE,smoke3di->nchars_compressed_smoke_full[i],SEEK_CUR);SKIP_SMOKE(SMOKE3DFILE);
      if(feof(SMOKE3DFILE)!=0){
        smoke3di->ntimes_full=i;
        smoke3di->ntimes=nframes_found_local;
        break;
...
void MergeSmoke3DColors(smoke3ddata *smoke3di){
  int i;
...
void MergeSmoke3DBlack(smoke3ddata *smoke3di){
  int i;
  int fire_index;
